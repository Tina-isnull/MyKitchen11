...
public class EditMenuActivity extends AppCompatActivity {
    ...
    private List<FoodFromWeb.Steps> stepList;
    private FoodDetails detials;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        detials = (FoodDetails) getIntent().getSerializableExtra("details");
        menu_material.setText(detials.getDetial().getIngredients());
        menuName.setText(detials.getDetial().getTitle());
        MenInfo.setText(detials.getDetial().getImtro());
        HttpRequestManager.displayImage(detials.getDetial().getBurden(), menuHeader);
        createConditon(detials.getDetial().getSteps());
    }
    ...
    public void upload(View view) {
        ...
        bean.update(this,detials.getId(), new UpdateListener() {
            @Override
            public void onSuccess() {
                Toast.makeText(EditMenuActivity.this, "修改成功", Toast.LENGTH_SHORT).show();
                finish();
            }

            @Override
            public void onFailure(int i, String s) {
                Toast.makeText(EditMenuActivity.this, "修改失败 " + s
     ...
     * 动态添加菜谱步骤
     */
    private void createConditon(List<FoodFromWeb.Steps> mStep) {
        if(mStep!=null&&mStep.size()!=0){
            for(FoodFromWeb.Steps item:mStep){
                //每次创建一个view
                ...
                EditText stepTv= (EditText) ll_limit.findViewById(R.id.et_add_menu_content_step);
                HttpRequestManager.displayImage(item.getImg(), imgStep);
                stepTv.setText(item.getStep());
                imgUrl.setText(item.getImg());
    ...
    public void getPicture(final ImageView img, Intent data, final int select) {
        ...
        bf.uploadblock(this, new UploadFileListener() {
            ...
            public void onSuccess() {
                //上传后的图片在服务器上的位置（网址）
                avatarUrl = bf.getFileUrl(EditMenuActivity.this);
            ...
            public void onFailure(int arg0, String arg1) {
                Toast.makeText(EditMenuActivity.this, "头像图片上传失败，错误代码：" + arg0 + "，" + arg1, Toast.LENGTH_SHORT).show();
            }
        });
    }
